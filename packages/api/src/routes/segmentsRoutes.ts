...
export const segmentsRoutesWrapper = (
    ...
    flagRoutes.put(
        ...
        async (req: Request, res: Response) => {
            console.log(
                "received segments update: " + JSON.stringify(req.body),
            );
            ...
            const alreadyInDb = await segmentsService.getSegment({
                key: segmentKey,
            });

            if (alreadyInDb) {
                alreadyInDb.updatedOn = dateHelper.utcNow().toJSDate();
                alreadyInDb.description = segmentDescription
                    ? segmentDescription
                    : alreadyInDb.description;
    ...
    flagRoutes.delete(
        ...
        async (req: Request, res: Response) => {
            console.log(
                "received segments delete: " + JSON.stringify(req.body),
            );
