...
export const sdkRoutesWrapper = (
    ...
    sdkRoutes.post(
        "/api/sdk/flags/:flagKey/evaluate",
        upload.any(),
        auth.isSdkAuthenticated,
        async (req: Request, res: Response) => {
            try {
                ...
                const resp = await sdkService.evaluateFlag(
                    flag,
                    flagContext,
                    correlationId,
                );
    ...
    sdkRoutes.get(
        "/api/sdk/segments/:segmentKey/conditions",
        auth.isSdkAuthenticated,
        async (req: Request, res: Response) => {
            try {
                const segmentKey = req.params.segmentKey as string;
                const conditions = await segmentsService.getConditionsBySegment(
                    segmentKey,
                );
