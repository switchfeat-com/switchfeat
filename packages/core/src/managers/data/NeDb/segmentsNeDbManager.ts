...
export const getSegments = async (): Promise<SegmentModel[]> =>
    neDbManager.segments!.asyncFind({});

export const getSegmentById = async (id: string): Promise<SegmentModel> =>
    neDbManager.segments!.asyncFindOne({
        _id: id,
    });

export const getSegmentByKey = async (key: string): Promise<SegmentModel> =>
    neDbManager.segments!.asyncFindOne({
        key: key,
    });
...
export const updateSegment = async (
    ...
    try {
        const response = await neDbManager.segments!.asyncUpdate(
            { _id: segment._id },
            {
                $set: {
...
export const deleteSegment = async (
    ...
    try {
        const response = await neDbManager.segments!.asyncRemove({
            _id: segment._id,
        });
        return !!response;
    } catch (ex) {
